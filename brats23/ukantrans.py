...
from PositionalEncoding import LearnedPositionalEncoding, LearnedPositionalEncoding_2
...
class UKAN(nn.Module):
    def __init__(self, num_classes, embed_dims=[128, 160, 256], drop_rate=0.1, drop_path_rate=0.1, norm_layer=nn.LayerNorm, depths=[1, 1, 1]):
        ...
        self.position_encoding_2 = LearnedPositionalEncoding_2()
        ...
        self.transformer_2 = TransformerModel(
            256,
            4,
            8,
            4096,
    ...
    def forward(self, x):
        ...
        out = self.norm3(out)
        out1 = out
        out1 = out1.reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
        t4 = out1
        transout = self.position_encoding(out)
        transout, intmd_x = self.transformer(transout)
        out = torch.add(out, transout)
        ...
        out = self.norm4(out)
        #print(out.shape)
        #transout = self.position_encoding_2(out)
        #transout, intmd_x = self.transformer_2(transout)
        #out = torch.add(out, transout)
        out = out.reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
