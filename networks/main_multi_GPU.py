...
def init_distributed_mode(config):
    if isinstance(config['gpu'], (list, tuple)):
        gpu_ids = ','.join(map(str, config['gpu']))
    else:
        gpu_ids = str(config['gpu'])
    ...
    if 'LOCAL_RANK' in os.environ:
        ...
        raise ValueError("no LOCAL_RANK")
...
def main_train(args):
    local_rank = init_distributed_mode(gpuconfig)
