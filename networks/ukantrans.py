...
class D_ViTLayer3D(nn.Module):
    def __init__(self, in_ch, out_ch,
                 ...
                 dropout=0.1,
                 downsample=False):
        super(D_ViTLayer3D, self).__init__()
        ...
        # Linear embedding layer for patch embeddings
        self.to_patch_embedding = nn.Linear(in_ch * patch_size ** 3, dim)

        # Positional embedding
        self.pos_embedding = nn.Parameter(torch.randn(1, 1, dim))  # 初始化为1, 将在后续根据num_patches扩展
        ...
        # MLP head to project the output back to desired channels
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, out_ch * patch_size ** 3)  # 将输出映射回patch_size**3 * out_ch
        )

        # Optional downsample layer (Max Pooling)
        if downsample:
    ...
    def forward(self, x):
        ...
        # Adjust pos_embedding size
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches, self.dim).to(x.device))

        # Rearrange x to patches: [B, in_ch, D, H, W] -> [B, num_patches, patch_size**3 * in_ch]
        x = rearrange(x, 'b c (d p1) (h p2) (w p3) -> b (d h w) (c p1 p2 p3)', p1=self.patch_size, p2=self.patch_size,
                      p3=self.patch_size)
        ...
        # MLP head to get the final output
        x = self.mlp_head(x)

        # Reshape back to [B, out_ch, D, H, W]
        x = rearrange(x, 'b (d h w) (c p1 p2 p3) -> b c (d p1) (h p2) (w p3)',
                      d=D // self.patch_size, h=H // self.patch_size, w=W // self.patch_size,
                      p1=self.patch_size, p2=self.patch_size, p3=self.patch_size, c=self.out_ch)

        # Downsample the output if downsample is True
...
class UKAN(nn.Module):
    def __init__(self, num_classes, embed_dims=[128, 160, 256], drop_rate=0.1, drop_path_rate=0.1, norm_layer=nn.LayerNorm, depths=[1, 1, 1]):
        ...
        '''
        self.encoder1 = D_ViTLayer3D(4, kan_input_dim // 8)
        self.encoder2 = D_ViTLayer3D(kan_input_dim // 8, kan_input_dim // 4)
        self.encoder3 = D_ViTLayer3D(kan_input_dim // 4, kan_input_dim)
